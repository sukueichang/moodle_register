<?php
/**
 * Persist submitted interactive calendar plan (Phase 4).
 *
...
require_once(__DIR__ . '/../classes/reservation_plan_validator.php');

use local_tm_course\permissions_manager;
use local_tm_course\reservation_plan_validator;
...
if (empty($result['ok']) || empty($result['blocks'])) {
    ...
    echo json_encode([
        'ok' => false,
        'error' => (string) ($result['error'] ?? 'reservation_calendar_error_invalid_block'),
    ]);
    exit;
}
