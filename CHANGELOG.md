...
### Added
- 業務在 Dashboard「學員服務」與左側導覽可進「申請追蹤」，查看自己送出的批改申請。

## [5.20.7] - 2026-09-11 - Dashboard 分組用詞

### Changed
- 首頁分組改為「既有課程」「客制專班」「學員服務」；按鈕名稱不變。

## [5.20.6] - 2026-09-11 - 被分派同事不可再分派

### Fixed
- 被分派同事即使 Moodle 角色帶有 `manage`，該單也不再顯示分派／駁回／復原。網站管理員仍可改派。

## [5.20.5] - 2026-09-11 - 被分派者待批改數字修正

### Fixed
- 被分派者若同時具 admin／manage，先前 N 只算未分派單，分派後數字是 0。改為優先顯示分派給自己、尚未評完的作業／測驗列數，並進入「指派給我」。

## [5.20.4] - 2026-09-11 - 同事待批改數字改為待評分列數

### Changed
- 被分派同事的「待批改 (N)」改為尚未評完的作業／測驗筆數，不再用申請單張數。

## [5.20.3] - 2026-09-11 - 分派負責人與改派紀錄

### Added
- 申請詳情狀態列顯示目前負責人與分派時間；改派後列出先前與目前分派紀錄。

## [5.20.2] - 2026-09-11 - 申請頁預覽成績與 Dashboard 名稱

### Changed
- Dashboard／導覽／申請頁名稱改為「作業/測驗批改申請」。
- 搜尋與勾選學員時即顯示該份作業／測驗是否已評分與分數，不必先送出申請。

## [5.20.1] - 2026-09-10 - 復原申請語言字串

### Fixed
- 補上已駁回／已取消單據「復原申請」的語言字串；升級外掛後會重載字串快取。
