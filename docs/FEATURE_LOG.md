...
## 2026-09-07 — 卡位報名：搜尋不到／完課掛錯帳號

- **需求：** 批次卡位學員已點名出席，但以真實 email 搜尋「課程相關紀錄」為 0；完課也可能掛在佔位帳號。
- **決策：** 維持 `userid`＝佔位、`linked_*`＝真實學員的雙帳號模型；搜尋／我的紀錄改同時認 `linked_email`／`linked_userid`；完課與 `mod_attendance` 共用同一 userid 解析。不在此版強制把 `userid` 換成真實帳號。
- **影響範圍：** `enrolment_manager` 搜尋／`get_user_records`、`attendance_manager::mark_attended`、`search.php`、version 5.19.5。
- **版本／狀態：** **5.19.5；進行中（待驗收）**
