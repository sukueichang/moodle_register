...
### 問題現象
- 學員 5/22 Beginner's 已完成（名冊顯示已完成，報名列仍為已核准）；8/31 被駁回後再批次報 Beginner's →「同課程其他場次有有效報名」。
- 使用者預期：駁回可重報；過去已上完的場次不應擋新場次。

### 根因判斷
- `error_course_enrolment_conflict` 將同課程任何 pending／approved／waitlisted 都視為衝突，未排除 `endtime` 已過的場次。
- 已駁回本身不在 active 狀態內，真正擋住的是 **5/22 仍為 approved 的歷史場次**。

### 解決方式
- 自主報名與批次報名的同課程互斥改為：僅當另有 **尚未結束**（`s.endtime > time()`）的有效報名時才擋。

### 預防準則
- 同課程互斥 =「未來／進行中不可重複佔名額」，不是「一生只能上一門」。
