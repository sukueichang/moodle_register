...
| 部署路徑 | `{moodle}/local/tm_course/` |
| 主要入口 | `/local/tm_course/index.php` |
| 設定 | `/admin/settings.php?section=local_tm_course` 與外掛內 `settings/` |
...
| 模組 | 主要路徑／類別 |
|------|----------------|
| 場次 CRUD／批次建班 | `admin/sessions.php`、`admin/edit_session.php`、`classes/session_manager.php` |
| 一般／批次報名 | `index.php`、`batch_enrol.php`、`classes/enrolment_manager.php` |
| 先修條件 | `classes/prerequisite_manager.php`、課程連動預設、場次規則 |
| 報名審核 | `admin/enrolments.php`、`admin/review_center.php` |
...
- 主色 Techman Blue `#005f7e`
- 強調色 Techman Green `#74b42a`

---

## 以下為完整規格正文（歷史 M 系列＋後續章節；與程式衝突時以「§0 現況總覽」與最新 FEATURE_LOG／BUGFIX_LOG 為準）
...
- **`docs/SPEC.md`**（本檔）為**行為與介面規格**的唯一規格來源（single source of truth for product spec）；根目錄 `spec.md` 僅為導向 stub。
- 任何行為調整必須先回寫本檔
- `README.md` 需維持與本檔的角色權限與近期行為一致（至少同步：Auto 時段規則、課程連動預設課時、角色權限區分）。
- **`docs/BUGFIX_LOG.md`**：累積使用者回報、除錯根因與已驗證解法（原 `lesson-learned.md`）。
- **`docs/FEATURE_LOG.md`**：需求與決策時間軸。
- **`docs/DEV_WORKFLOW.md`**：提需求／修 bug／驗收後 PR‧push 的 SOP。
...
- 使用者回報問題、根因與已驗證解法，集中維護於 **`docs/BUGFIX_LOG.md`**（原 `lesson-learned.md`）。
- **`docs/SPEC.md`** 描述應有行為與介面規格；**`docs/BUGFIX_LOG.md`** 描述曾發生的缺陷與修復策略（含 Moodle 平台限制、部署語系、AJAX／FullCalendar、DB 命名等），避免同一教訓重複維護兩份。
- 開發與除錯時應優先查閱 **`docs/BUGFIX_LOG.md`**（例如：`AJAX_SCRIPT` 須 `$PAGE->set_context()`、`get_records_sql` 第一欄須唯一、資料表名 ≤28 字元、FullCalendar `eventsSet` 內不可遞迴改事件、課前檢核與審核狀態顯示等）。
- 需求決策見 **`docs/FEATURE_LOG.md`**；流程見 **`docs/DEV_WORKFLOW.md`**。
...
本章為 **閱讀用索引**：細部行為仍以 §1～§54 與各節「已實作更新」為準；維運除錯請搭配 **`docs/BUGFIX_LOG.md`**。
...
- 呼叫 `fullname()` 前須載入完整 `user` 列（`*`），避免 AJAX／redirect 時 debugging 污染 JSON（見 `docs/BUGFIX_LOG.md`）。
...
- 除錯細節與部署踩雷記錄維護於 **`docs/BUGFIX_LOG.md`**（`get_records_sql` 首欄唯一、證書 course-level 語意等）。
...
**關單：** Admin 可駁回（必填原因）。業務僅在未分派且尚無任何成績時可取消。已分派或已有成績只有 admin 能關。
